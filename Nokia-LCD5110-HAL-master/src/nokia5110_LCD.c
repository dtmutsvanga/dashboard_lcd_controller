...
void LCD_init(){
  ...
  LCD_write(LCD_DISPLAY_C0NTRAST_CMD|contrast, LCD_COMMAND); //set LCD Vop(Contrast).
  LCD_write(0x04, LCD_COMMAND); //set temp coefficent.
  LCD_write(0x14, LCD_COMMAND); //LCD bias mode 1:48.
...
void LCD_putCyrillicChar(char c) {
...
void print_char(char c, const uint8_t table[][6]) {

for(int i = 0; i < 6; i++)
    {
      if(lcd.inverttext != true)
        LCD_write(table[c][i], LCD_DATA);
      else
        LCD_write(~(table[c][i]), LCD_DATA);
    }
}
...
void dashb_to_lcd_char( char c){
    /*Ascii*/
    if (((c >= 0xA0) && (c <= 0xAF)) || ((c >= 0xB0) && (c <= 0xDA)) || ((c >= 0xE0) && (c <= 0xFA)))
    {
      c = 0x7F&(c) - 0x20;
      print_char(c, ASCII);
    }
    ...
    /* Cyrillic Á - Ø*/
    else if (c >= 0x44 && c <= 0x4F)
    {
      c -= 0x44;
      print_char(c, cyrilic);
    }
    
   /* Cyrillic  á - ø */
    else if(c >= 0x54 && c <= 0x5F)
    {
      c = c - 0x54 + 12;
      print_char(c, cyrilic);
    }
   /* Cyrillic  Ù - å*/
    else if(c >= 0x64 && c < 0x6E)
    {
      c =c - 0x64+24;
      print_char(c, cyrilic);
    }
    /* Cyrillic  ù - ò*/
    else if(c >= 0x74 && c <= 0x7D)
    {
      c = c - 0x74+35;
      print_char(c, cyrilic);
    }
